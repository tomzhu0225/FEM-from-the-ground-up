...
def fractal_mesh_quad(nelemsx,start,size,degree): 
    ...
    if start+size*4>nelemsx:
        ...
        return
    xmin, xmax, ymin, ymax = 0.0, 2.0, 0.0, 4.0
...
def plane_wave(wavenumber, degree, a, plot_type="2d",alignment="right"):
    ...
    if degree > 2:
        ...
        if alignment=="right":
            node_coords, p_elem2nodes, elem2nodes, boundary_idx = fractal_mesh_tri(64, 0, 8, degree)
        else:
            node_coords, p_elem2nodes, elem2nodes, boundary_idx = fractal_mesh_quad(64, 0, 8, degree)
            node_coords, p_elem2nodes, elem2nodes=quad2tri(node_coords, p_elem2nodes, elem2nodes)
    ...
    nodes_on_boundary = nodes_on_south+nodes_on_north

    # Set Dirichlet boundary conditions
    values_at_nodes_on_boundary = np.zeros((nnodes, 1), dtype=np.complex128)
    values_at_nodes_on_boundary[nodes_on_north] = -a
    values_at_nodes_on_boundary[nodes_on_south] = 0
...
if __name__ == '__main__':
   ...
   plane_wave(5, 0, 1, plot_type="2d",alignment="right")