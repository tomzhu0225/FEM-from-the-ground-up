...
#     """Apply Dirichlet boundary conditions to K and M matrices.
    
#     Parameters:
#         K (ndarray): Stiffness matrix
#         M (ndarray): Mass matrix
#         dirichlet_nodes (list): List of node indices with Dirichlet conditions
#         dirichlet_values (list): List of prescribed values at the Dirichlet nodes
#     """
    
#     for node in zip(dirichlet_nodes):
#         # Zero out the row and column in K
#         K[node, :] = 0
#         K[:, node] = 0
#         K[node, node] = 1

#         # Zero out the row and column in M
#         M[node, :] = 0
#         M[:, node] = 0
#         # Optionally: M[node, node] = 1

#     return K, M
def _set_dirichlet_condition_eig(K, M, dirichlet_nodes):
    """Apply Dirichlet boundary conditions to K and M matrices by removing rows and columns.
    ...
    # Sort dirichlet_nodes in descending order to avoid index issues when deleting
...
def _plot_contourf(nelems, p_elem2nodes, elem2nodes, node_coords, node_data, **kwargs):
    ...
    triangles = [elem2nodes[p_elem2nodes[i]:p_elem2nodes[i + 1]] for i in range(nelems)]

    # creates triangulation
    triang = matplotlib.tri.Triangulation(x, y, triangles)
    
    # Analyze triangulation to identify triangles that are inside holes
    ...
    axs.plot_trisurf(triang, z, cmap='viridis', edgecolor='none')
    matplotlib.pyplot.show()
def _plot_2d_contour(nelems, p_elem2nodes, elem2nodes, node_coords, node_data, **kwargs):
    ...
    levels = numpy.linspace(min(z), max(z), 50)  # setting contour levels
    contour = ax.tricontourf(triang, z, levels=levels, cmap='jet', **kwargs)
    fig.colorbar(contour)


    ax.set_aspect('equal')
    matplotlib.pyplot.show()
def _plot_2d_contour_eig(k,nelems, p_elem2nodes, elem2nodes, node_coords, node_data, **kwargs):
    """Plot node data parameter on the mesh as 2D contours."""
    x = node_coords[:, 0]
    y = node_coords[:, 1]
    z = node_data
    ...
    contour = ax.tricontourf(triang, z, levels=levels, cmap='jet', **kwargs)
    fig.colorbar(contour)
    ax.set_title("Eigenmode K="+str(k))
